...
describe("Modifiers - protección y validación", function () {
  async function deployFixture() {
    ...
    const EjercicioDos = await ethers.getContractFactory("EjercicioDos");
  ...
  it("Verifica acceso a método - admin", async function () {
    ...
    await setBalance(admin, ethers.BigNumber.from("10000000000000000000"));
  ...
  it("Verifica lista blanca", async function () {
    ...
    await ejercicioDos
      .connect(signerAdmin)
      .incluirEnListaBlanca(alice.address);
