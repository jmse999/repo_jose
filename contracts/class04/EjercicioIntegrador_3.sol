 ...
 * - npx hardhat test test/EjercicioIntegrador_3.ts
 */

contract EjercicioTres {
    ...
    modifier soloAdmin(){
        require(msg.sender==admin,"no es admi");
        _;
    }
    ...
    mapping(address admin=>uint256 cantidad) balances;
    ...
    function depositar(uint256 _cantidad) public {
        balances[admin]+= _cantidad;
        balanceTotal+= _cantidad;
        emit Deposit(admin,_cantidad);
    ...
    function retirar(uint256 _cantidad) public {
        if (balances[admin] >= _cantidad) revert SaldoInsuficiente();
        balances[admin]-= _cantidad;
        balanceTotal-= _cantidad;
        emit Withdraw(admin,_cantidad);
    }
    ...
    function transferir(address _destinatario, uint256 _cantidad) public {
        ...
        balances[admin]-= _cantidad;
        balanceTotal-= _cantidad;
        emit Transfer(admin, _destinatario,_cantidad);
    }
